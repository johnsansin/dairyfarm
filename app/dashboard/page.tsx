...
import FarmCopilot from '../../components/farm-copilot';
...
export default function Dashboard(){
 ...
 if(loading)return <div className="loading" role="status">{t('Opening your workspace…','آپ کا ورک اسپیس کھل رہا ہے…')}</div>;
 return <div className={`dashboard-shell${farms.length?' has-farm':''}${menuOpen?' nav-open':''}`}>
  <Header ur={ur} toggle={toggle} dark={dark} toggleDark={toggleDark} user={user} farms={farms} selected={selected} onSelect={id=>{setSelected(id);setSelectedOrg(farms.find(f=>f.id===id)?.organization_id||'')}} base={base} busy={busy} onLogout={()=>void logout()} onProfile={()=>router.push('/account')} onSettings={()=>setTab('settings')} menuOpen={menuOpen} onMenu={()=>setMenuOpen(o=>!o)}/>
  {selected&&<FarmCopilot base={base(selected)} ur={ur}/>} 
  <div className="workspace-main">
   {error&&<div className="form-error" role="alert">{error}</div>}
   {user&&!farms.length&&management}
   {user&&farms.length>0&&<FarmWorkspace management={management} key={selected} farm={{id:selected,name:farm?.name||'',role:farm?.role||'OWNER',currency:farm?.currency||'PKR'}} ur={ur} requestedTab={tab} onTabUsed={()=>setTab(undefined)} focusAnimalId={qAnimal||undefined} menuOpen={menuOpen} onMenu={()=>setMenuOpen(o=>!o)}/>} 
   </div>
