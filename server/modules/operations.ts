...
operations.get('/summary',async(_req,res)=>{
 ...
 const trend=(await db.query("SELECT date::text,sum(quantity)::text AS quantity FROM milk WHERE farm_id=$1 AND date>=(now() AT TIME ZONE 'Asia/Karachi')::date-29 GROUP BY date ORDER BY date",[farm])).rows;
