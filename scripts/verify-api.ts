...
async function main(){
 ...
 const f=await call('/farms','POST',{name:'API verification farm',city:'Lahore',currency:'PKR'},cookieA);assert.equal(f.status,201);const farm=await f.json();
 ...
 assert.equal((await call('/auth/login','POST',{email:userA.email,password})).status,200);
